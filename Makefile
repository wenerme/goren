...
docker-build:
	...
	${CONTAINER_TOOL} run --rm ${IMAGE} /bin/cat /goren > bin/goren
...
docker-test:
    ...
    -v ${ROOT_DIR}/config.yml:/etc/goren/config.yml:ro \
    -v ${ROOT_DIR}/tests:/opt/tests:ro \
    ${REGISTRY}/${AUTHOR}/${NAME}:${VERSION} --config /etc/goren/config.yml
